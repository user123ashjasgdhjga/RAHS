...
def multi_simulate(args):
    # [新增] 将 args 的开关同步给 sim_config
    sim_config.enable_throughput_noise = args.stress_noise
    sim_config.enable_performance_cliffs = args.stress_cliff
    sim_config.enable_cross_job_interference = args.stress_interfere
    # ======= 新增：同步微观层开关 =======
    sim_config.enable_micro_adjustment = not args.disable_micro

    # ===== 新增：同步风险调整效用模块三超参数 =====
    # (1) δ: perturbation factor
    if args.risk_delta is not None:
        sim_config.batch_size_perturbation = args.risk_delta
    # (2) γ: risk aversion coefficient
    if args.risk_gamma is not None:
        sim_config.risk_aversion_coefficient = args.risk_gamma
    # (3) β: retention factor
    if args.risk_beta is not None:
        sim_config.risk_retention_factor = args.risk_beta

    # 打印当前生效的三参数，方便实验记录
    print(f"\n{'=' * 60}")
    print(f"[敏感性实验] 风险调整效用模块超参数:")
    print(f"  δ (perturbation factor)     = {sim_config.batch_size_perturbation}")
    print(f"  γ (risk aversion coeff.)    = {sim_config.risk_aversion_coefficient}")
    print(f"  β (retention factor)        = {sim_config.risk_retention_factor}")
    print(f"{'=' * 60}\n")
    ...
    # ===== 新增：在输出日志中记录实验超参数 =====
    if args.output:
        # 写入超参数到日志文件同目录，方便事后分析
        params_path = args.output.replace('.log', '_risk_params.json') if args.output.endswith('.log') \
            else args.output + '/risk_params.json'
        import json as _json
        risk_params = {
            'delta': sim_config.batch_size_perturbation,
            'gamma': sim_config.risk_aversion_coefficient,
            'beta': sim_config.risk_retention_factor,
        }
        try:
            with open(params_path, 'w') as pf:
                _json.dump(risk_params, pf, indent=2)
        except Exception:
            pass  # 输出路径是目录时忽略
...
if __name__ == "__main__":
    ...
    parser.add_argument("--risk_delta", type=float, default=None,
                        help="δ: perturbation factor for sensitivity calculation (default: 0.05)")
    parser.add_argument("--risk_gamma", type=float, default=None,
                    help="γ: risk aversion coefficient (default: 0.15)")
    parser.add_argument("--risk_beta", type=float, default=None,
                    help="β: retention factor, minimum goodput fraction to keep (default: 0.9)")
    ...
    # [新增命令行参数]
    parser.add_argument("--stress_noise", action='store_true', help="Enable throughput noise")
    parser.add_argument("--stress_cliff", action='store_true', help="Enable performance cliffs")
    parser.add_argument("--stress_interfere", action='store_true', help="Enable cross-job interference")
    # ======= 新增：微观层消融实验开关 =======
    parser.add_argument("--disable_micro", action='store_true', help="Disable micro-layer batch size adjustment")
